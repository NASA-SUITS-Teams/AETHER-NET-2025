...
class ErrorStateDisplay(QWidget):
    ...
    def update_error_state(self, state, error_id):
        self.error_states[error_id] = state
        button = self.buttons[error_id]
        button.setStyleSheet(f"background-color: {'red' if state else 'green'}; color: white;")
        button.setChecked(state)
        state_text = "ON" if state else "OFF"
        self.chat_box.append(f"Error {error_id}: {state_text}")


if __name__ == "__main__":
    app = QApplication(sys.argv)
    window = ErrorStateDisplay()
    window.show()
    sys.exit(app.exec_())
