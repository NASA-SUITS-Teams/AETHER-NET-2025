...
class LEDIndicator(QFrame):
    ...
    def set_color(self, color: str):
        ...
        elif self.label_text.lower() == "pump" and color == "red":
            self.blink = True
            self.blink_timer.start(500)
        elif self.label_text.lower() == "fan" and color == "red":
            self.blink = True
            self.blink_timer.start(500)
        else:
            self.blink = False
...
# The Server polling; continuous
shared_results = {}

# Thread-safe lock for data access
results_lock = Lock()

# Polls server and decodes data to store in shared_results
def udp_polling_loop():
    SERVER_IP = "172.17.218.125"
    SERVER_PORT = 14141
    team_num = 0

    # Keys that should be interpreted as floats
    float_outputs = (
        set(range(17, 23)) |
        set(range(27, 37)) |
        set(range(38, 48)) |
    ...
    while True:
        ...
        for command in range(1, 119):
            try:
                ...
                if command in float_outputs:
                    final = round(float_val, 2)  # Keeps two decimal places
                else:
                    final = int_val

                # ✅ Log the result to the file
                logger.info("", extra={"command": command, "value": final})
                #if (command in range (26, 48)):
                    #print(command, " - float: ", float_val, " Final = ", final)
        ...
        # Update global results dictionary with lock
        with results_lock:
...
# Bind new read function to class
